...
{
    /// <summary>
    /// Nuclear Option is URP — Built-in Standard from .nobp does not draw.
    /// Clone live URP Lit from AShM shell; Cull Off for Blender normals.
    /// </summary>
    internal static class VisualShader
    {
        ...
        internal static Material Make(string name) => Make(name, cull: 0f);
        ...
        private static Shader Resolve()
        {
            ...
            for (int i = 0; i < scene.Length; i++)
            {
                ...
                _template = mat;
                _lit = mat.shader;
                CrosswimPlugin.ModLog?.LogInfo($"VisualShader scene '{_lit.name}' from '{r.name}'");
                return _lit;
            }
            ...
            {
                _lit = found;
                CrosswimPlugin.ModLog?.LogInfo($"VisualShader Shader.Find '{_lit.name}'");
                return _lit;
            }
            ...
            CrosswimPlugin.ModLog?.LogWarning($"VisualShader fallback '{_lit.name}' (URP Lit missing)");
            return _lit;
        }
        ...
        internal static void EnableBaseMap(Material mat)
        {
            if (mat == null)
                return;
            mat.EnableKeyword("_BASEMAP");
